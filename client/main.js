...
async function fetchProducts(){
    ...
    .then(products => {
        ...
        products.forEach(prod => {
            // <td><img class="product-image" src="${prod.image}"/></td>
            html += `
                <tr id=${prod.id}>
                    ...
                    <td class="row-data"><button onclick="addToCartBtn()" > Add To Cart</button></td>
                </tr>
            `
        });
...
async function createCart(cartElem){
    const response = await fetch('http://localhost:3000/cart', {
        ...
        body: JSON.stringify({productId: cartElem.productId,
...
async function addToCartBtn(){
    ...
    const cartElem = {productId: rowId,
...
async function onClickDecrease(){
    let cartId = event.target.parentNode.parentNode.id.split('-')[1];
...
function cartsTable(carts){ 
    ...
    document.getElementById('cart').innerHTML += `<td id="total" colspan="4"> Total: ${total} </td>`;
